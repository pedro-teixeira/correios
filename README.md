...
- **Etiquetas de Frete:** A opção de imprimir o rótulo da postagem é exibida na tela de entrega do Magento, após a etapa acima (requisição do código de postagem). A impressão baixa um róluto em PDF no padrão Correios (105 x 150 mm), que foi gerado na etapa anterior. Ou seja se precisar alterar alguma informação na etiqueta, é necessário remover o código de postagem anterior, e criar uma nova etiqueta de frete. As chancelas para impressão devem ser previamente baixadas, através da atualização dos serviços, na tela de configurações do módulo.
